# P&ID를 검색 가능한 데이터로 정리하는 방법

> 이 단원에서는 P&ID의 모든 장비와 연결 관계를 추출하지 않습니다. 도면이 무엇을 나타내는지 빠르게 파악하고, 주요 구역과 태그를 다시 찾을 수 있는 수준으로 정리합니다.

앞 단원에서는 위치 상자를 사용해 이미지의 특정 위치를 다시 찾아가는 방법을 배웠습니다. 이제 같은 방법을 도면 전체에 적용해 보겠습니다. 빈 JSON 파일에서 시작해 도면의 기본정보, 한 줄 요약, 큰 구역과 주요 태그를 한 단계씩 추가합니다.

완성된 결과는 P&ID를 완전히 디지털화한 모델이 아닙니다. 사람이거나 AI 에이전트가 도면을 열기 전에 `어떤 도면인지`, `어떤 큰 구역이 있는지`, `어디를 먼저 확대할지` 판단하는 데 사용하는 간단한 색인입니다.
...
![도면번호, 제목, revision과 sheet가 표시된 제목란](../assets/regions/R06-title-block.png)
...
이제 이미지를 열지 않아도 도면번호, 제목, revision과 sheet를 확인할 수 있습니다. `5/5`는 같은 계통을 설명하는 다섯 장 중 다섯 번째 시트라는 뜻입니다.

## 3단계: 도면 전체를 한 문장으로 요약합니다

제목만으로는 이 시트에 어떤 설비가 나타나는지 충분히 알기 어렵습니다. 작은 태그를 읽으려 하지 말고, 전체 이미지에서 반복되는 큰 구조를 살펴봅니다.

![세 개의 Reception Bin 계통이 반복되는 교육용 전체 P&ID](../assets/overview/sample-pid-overview-1600.png)
...
![전체 P&ID 위에 R01부터 R06까지 큰 구역을 표시한 지도](../assets/bbox/pid-major-regions.png)
...
![Reception Bin 1과 하부 설비를 포함한 큰 구역](../assets/regions/R02-reception-bin-1.png)
...
구역은 서로 조금 겹쳐도 괜찮습니다. 예를 들어 하부 연결을 이해하려면 R02, R03, R04의 아래쪽과 R05의 위쪽이 함께 보여야 할 수 있습니다. 중요한 것은 도면을 빈틈없이 조각내는 것이 아니라, 다음 탐색의 출발점을 만드는 것입니다.

## 5단계: 구역마다 짧은 설명을 붙입니다

`name`만으로 구역의 범위를 충분히 설명하기 어려울 때에는 `summary`를 추가합니다. 앞에서 만든 R03을 다음과 같이 고도화할 수 있습니다.
...
![Reception Bin 2의 출구와 Vibro Feeder 태그를 확인할 수 있는 확대 이미지](../assets/regions/feeder-c-area.png)
...
여기서는 태그 사이의 모든 연결 관계를 만들지 않습니다. `어떤 태그가 이 도면에 있는가`와 `어느 큰 구역에서 다시 확인할 수 있는가`에 답하는 것이 목표입니다.
...
지금은 P&ID 한 장만 정리했습니다. 같은 형식으로 여러 도면의 `title`, `summary`, `regions`, `tags`를 저장하면 AI 에이전트가 작업을 시작하기 전에 각 도면의 의미를 빠르게 파악하는 데 활용할 수 있습니다.
