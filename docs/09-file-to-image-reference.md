...
> 60분 수업에서는 강사가 이 과정을 시범으로 보여 줄 수 있습니다. 참가자가 직접 실행할지는 강사가 수준에 맞춰 선택합니다.
...
`텍스트 0줄`은 파일이 비었다는 뜻이 아니다. 글자가 페이지 그림의 일부로 존재해 일반 텍스트 추출기가 읽을 수 없다는 뜻입니다.
...
아래 명령은 터미널을 사용할 수 있을 때 선택적으로 실행합니다. 명령어를 이해하거나 외우는 것은 핵심 학습 목표가 아니다.
