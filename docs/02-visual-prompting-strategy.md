...
앞 단원의 실제 CLI 실험에서는 전체 이미지에서 작게 보이던 태그를 부분 확대 이미지에서 훨씬 정확하게 읽었습니다. 전체 이미지는 위치를 찾고, 부분 확대 이미지는 정확한 값을 읽는 데 사용하는 이유를 살펴보겠습니다.

## 먼저 실제 결과를 다시 봅니다

앞 단원에서 본 수동 근거 검토 점수를 필요한 부분만 표로 다시 정리하면 다음과 같습니다.

| 모델 | 전체+짧은 질문 | 부분 확대+짧은 질문 | 부분 확대+단계별 질문 |
|---|---:|---:|---:|
| Claude Sonnet | 3/4 | 4/4 | 4/4 |
| Codex Luna | 2/4 | 4/4 | 4/4 |
| Codex Terra | 2/4 | 4/4 | 3/4 |

가운데 열을 보면 Feeder #C 주변 확대 이미지와 짧은 질문을 사용했을 때 세 모델이 모두 4점을 받았습니다.

전체 이미지에서 Sonnet은 Feeder #C의 전동기를 `M-TP16`으로 잘못 읽었습니다. Luna는 Feeder 태그와 전동기 태그를 모두 잘못 읽었고, Terra는 두 태그를 확인하지 못했다고 남겼습니다.
...
오른쪽 열에서는 같은 부분 확대 이미지에 단계별 확인 순서를 추가했습니다. Sonnet과 Luna는 4점을 유지했지만 Terra는 오른쪽 Feeder의 `03B`를 Unit 1 값과 섞어 읽어 3점으로 낮아졌습니다.

따라서 이번 단원의 출발점은 다음 두 문장입니다.

> 부분 확대 이미지는 확대경과 같습니다. 정답에 필요한 글자와 연결이 모두 들어오도록 범위를 정해야 합니다.

> 이미지와 짧은 질문만으로 답이 명확하다면 지침을 더 늘리지 않습니다. 실제 오류가 확인되었을 때 필요한 규칙만 추가합니다.
...
새 검증 가능 질문 실험에서 Claude Code Sonnet이 보고한 전체 비용은 실행당 약 $0.050~$0.105였습니다. 이 값에는 이미지뿐 아니라 Claude Code의 시스템 지침, 파일 읽기 도구, cache 작성과 읽기, 추론과 출력 token이 포함됩니다.
