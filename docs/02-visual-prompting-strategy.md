...
이 순서는 모든 문제에 무조건 적용해야 하는 규칙은 아닙니다. 전체 도면의 제목만 묻는 질문이라면 확대가 필요하지 않습니다. 반대로 작은 태그 하나를 전사하는 질문이라면 처음부터 고해상도 crop이 더 효율적일 수 있습니다.
...
Q005에서는 Reception Bin 아래쪽에 Vibro Feeder가 반복된다는 사실을 확인합니다. 그러나 `#C`라는 작은 글자를 전체 이미지에서 확정하려고 하지 않습니다.

전체 이미지를 사용할 때에는 다음과 같이 요청할 수 있습니다.

```text
이 이미지는 도면 전체의 위치를 파악하기 위한 이미지입니다.
작은 태그를 추측해서 읽지 말고, Q005의 Vibro Feeder #C를 확인하려면
어느 구역을 확대해야 하는지 설명해 주세요.
```

## 2단계: 질문과 관련된 구역만 확대합니다

Q005에서는 Reception Bin 2 아래쪽에서 Feeder #C가 보이는 부분만 잘라 크게 만듭니다. 프로젝트 데이터에서는 이 구역을 R03이라고 부릅니다.
...
이제 작은 글자는 보이지만 제목란은 보이지 않습니다. 따라서 이 crop만 보고 전체 계통 이름이나 현재 도면번호를 새로 판단하면 안 됩니다. 전체 도면에서 이미 확인한 문맥과 연결하여 읽어야 합니다.

## 3단계: 한 번에 하나의 판단을 요청합니다

확대 이미지 한 장에 장비, 모든 계기, 연결 관계와 고장 원인을 한꺼번에 물으면 어떤 근거에서 오류가 생겼는지 알기 어렵습니다.

Q005에서는 다음처럼 나누어 확인합니다.
...
이미지만으로 확정할 수 없는 component 의미는 확인하지 못함으로 남겨 주세요.
```
...
이렇게 질문을 나누면 `태그는 맞았지만 component 해석이 틀렸습니다`처럼 오류가 발생한 단계를 찾을 수 있습니다.
...
- Crop에 보인 off-page reference는 현재 도면번호가 아니라 다른 시트를 가리키는 표기입니까?
...
그러나 Q005처럼 질문의 장비가 어느 반복 구조에 있을지 추정할 수 있다면, 기계적으로 네 조각을 모두 보는 것보다 R03처럼 질문 중심의 crop을 만드는 편이 더 직접적입니다.

## Trillion Labs 내부 사례에서는 어떤 차이가 있었나요?

Trillion Labs에서는 P&ID 이해 task에 같은 모델을 사용하면서 이미지를 전달하는 방식을 바꾸어 비교했습니다.
...
이 수치는 제공된 Trillion Labs 내부 메모를 바탕으로 합니다. Judge도 같은 Gemini 모델이며, 원본 메모가 bias 가능성을 언급합니다. Export에는 표본 수와 불확실성 구간이 포함되어 있지 않으므로 독립적인 범용 benchmark가 아니라 이미지 입력 설계의 중요성을 보여 주는 내부 사례로 해석합니다.
...
## 실제 CLI run의 비용은 왜 더 큰가요?

새 단일 도면 이해 실험에서 Claude Code Sonnet이 보고한 전체 비용은 run당 약 $0.058~$0.100이었습니다. 이 값에는 이미지뿐 아니라 Claude Code의 시스템 지침, Read tool, cache 작성과 읽기, reasoning과 출력 token이 포함됩니다.
...
- `CLI reported usage/cost`: CLI가 직접 보고한 값입니다.
- `API-equivalent estimate`: 공식 API token 단가로 환산한 비교용 추정값입니다.
