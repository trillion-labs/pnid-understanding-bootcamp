...
아래 실습은 실제 Codex 또는 Claude Code를 호출하고 결과를 `output/첫-실험/`에 저장합니다. 두 명령 중 사용할 도구 하나를 골라 실행하면 됩니다. 두 도구를 비교하고 싶을 때만 둘 다 실행합니다.

시작하기 전에 `codex --version` 또는 `claude --version`으로 설치 여부를 확인하세요. 처음 사용한다면 해당 명령을 한 번 실행해 로그인을 마친 뒤 실험을 시작합니다.

각 단계에서는 **명령을 먼저 실행하고 답변을 읽어 본 뒤** 결과 토글을 열어 비교합니다. 실행 명령에 달린 주석을 보면 모델, 이미지, 질문, 세션과 결과 파일이 어떻게 고정되는지 확인할 수 있습니다.

> `codex exec`와 `claude -p`는 실험 조건을 고정하고 결과를 파일로 남기기 위한 방식입니다. 원한다면 `codex` 또는 `claude`를 실행해 대화형으로 같은 이미지와 질문을 전달해도 됩니다. 다만 조건을 공정하게 비교하려면 실험마다 새 대화를 시작하세요.

> **대화형으로 실행하고 싶나요?**
>
> - **Codex:** 프로젝트 폴더에서 `codex --image 실습자료/이미지/전체-도면.png`를 실행한 뒤 `실습자료/프롬프트/기본-질문.txt`의 질문을 붙여 넣습니다. 입력창에 이미지를 직접 붙여 넣어도 됩니다.
> - **Claude Code:** 프로젝트 폴더에서 `claude`를 실행합니다. 입력창에 `@실습자료/이미지/전체-도면.png`를 입력해 파일을 선택한 뒤 같은 질문을 붙여 넣습니다.
>
> 두 번째 실험에서는 이미지는 그대로 두고 단계별 질문을 사용합니다. 세 번째 실험에서는 질문을 원래대로 돌리고 이미지만 `실습자료/이미지/feeder-c-확대.png`로 바꿉니다. 자세한 사용법은 [OpenAI 이미지 입력 안내](https://learn.chatgpt.com/docs/image-inputs)와 [Claude Code CLI 안내](https://docs.anthropic.com/en/docs/claude-code/cli-usage)를 참고하세요.

## 실험 1: 전체를 한눈에 보는 이미지로 시작합니다

도면 제목, 범례와 전체 연결이 한 화면에 보이는 이미지를 제공합니다. Reception Bin 2가 어느 부분에 있는지는 알 수 있지만 작은 태그는 매우 작게 보입니다.

터미널에서 Codex나 Claude Code 명령 중 하나를 실행하고, 네 항목 가운데 몇 개를 정확히 읽었는지 먼저 세어 보세요.
...
```bash
# Opus 등 다른 Claude 모델을 쓰려면 아래 --model 값을 바꾹니다.
# Claude Sonnet에 전체 도면과 기본 질문을 전달합니다.
claude -p \
  ...
  --max-budget-usd 0.30 \
  --output-format text \
  | tee output/첫-실험/1-claude-sonnet-전체-기본.txt
```
...
| `--model sonnet` | Claude Sonnet을 사용합니다. 계정에서 사용 가능한 다른 별칭이나 전체 모델명으로 바꿀 수 있습니다. |
...
claude -p \
  ...
  --max-budget-usd 0.30 \
  --output-format text \
  | tee output/첫-실험/2-claude-sonnet-전체-단계별.txt
```
...
claude -p \
  ...
  --max-budget-usd 0.30 \
  --output-format text \
  | tee output/첫-실험/3-claude-sonnet-확대-기본.txt
```
...
<details class="practice-result">
  ...
  <div class="practice-result__body">
    ...
    <p>문제는 전체 도면 자체가 아닙니다. 작은 글자를 읽을 만한 정보가 남아 있는지, 필요한 곳을 확대할 수 있는지가 관건입니다. 이 단원에서는 현업의 검토 순서에 맞춰 전체에서 위치를 찾고 관련 구역을 확대합니다.</p>
  </div>
</details>
...
셋째, `더 자세히 보라`는 프롬프트는 축소된 이미지에서 작게 표현된 세부 정보를 선명하게 만들지 못합니다.

넷째, 확인한 확대 구역은 답변의 근거로 남기면 다른 사람이 같은 위치를 빠르게 검토할 수 있습니다.

이번 실험에서 확인한 내용은 아래와 같습니다.

> 작은 글자 문제를 프롬프트만으로 해결하려 하지 않습니다. 전체 도면에서 위치를 찾고, 원본을 확대해 값을 읽고, 확인한 구역을 답변의 근거로 남깁니다.
