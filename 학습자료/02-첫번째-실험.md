...
- `full`: 전체 도면 이미지 `실습자료/이미지/전체-도면.png`를 입력합니다.
- 질문: `실습자료/프롬프트/기본-질문.txt`를 사용합니다.
...
```bash
./scripts/run-workbook-cli.sh codex-luna full-guided
```
...
- `zoom`: 부분 확대 이미지 `실습자료/이미지/feeder-c-확대.png`를 입력합니다.
- 질문: 첫 번째 실행과 같은 `실습자료/프롬프트/기본-질문.txt`를 사용합니다.
...
기존 단계별 질문까지 포함한 모델별 답변은 [첫 번째 실험의 모델별 상세 답변](부록/모델별-답변.md)에서 확인할 수 있습니다.
