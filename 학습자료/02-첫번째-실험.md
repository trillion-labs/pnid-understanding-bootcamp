...
앞 단원에서는 Reception Bin 2의 A측 출구에서 선을 따라 Feeder #C와 전동기까지 확인했습니다. 이번에는 같은 작업을 Claude Sonnet, Codex Luna, Codex Terra에 맡긴 결과를 비교하고 직접 재현합니다.
...
> 외부 검색, 코드 실행, 줌·OCR 등 도구를 사용하지 말고, 제공된 이미지의 현재 해상도에서 한 번에 답변해 주세요. 답변은 위 네 항목과 연결 관계 설명만 포함해 주세요.
...
각 실험은 새 대화에서 시작하고 같은 모델을 유지합니다. 실험 1과 2는 전체 이미지를 유지하고 질문만 바꿉니다. 실험 1과 3은 짧은 질문을 유지하고 이미지만 바꿉니다.
...
첨부한 이미지의 미리보기, 질문과 선택 모델을 전송 전에 함께 확인합니다. 모델 이름은 계정과 앱 버전에 따라 다르게 보일 수 있습니다.
...
기록된 결과를 재현하려면 Claude는 Sonnet, Codex는 GPT-5.6 Luna 또는 Terra를 사용합니다. 다른 모델을 선택했다면 기록된 점수의 재현이 아닌 새 실험으로 구분하고, 세 조건을 비교하는 동안 같은 모델을 유지하세요.
...
| `--model sonnet` | Claude Sonnet을 사용합니다. 세 실험에서 같은 모델을 유지합니다. |
...
아래 프롬프트를 그대로 복사해 전체 도면 이미지와 함께 새 대화에 입력하세요.

```text
이 P&ID에서 RECEPTION BIN 2의 A측 하부 출구와 연결된 VIBRO FEEDER #C를 다음 순서로 확인해 주세요.

1. `RECEPTION BIN 2`를 찾고 바로 아래에 적힌 장비 태그를 그대로 읽습니다.
2. Bin 하부의 `A` 표시에서 선을 아래로 따라가며 중간의 밸브 번호를 읽습니다.
3. 같은 선에 연결된 `VIBRO FEEDER #C`를 찾고, Unit 1과 Unit 2 표기 옆의 장비 태그를 그대로 읽습니다.
4. Feeder #C 바로 옆의 원형 전동기 심벌 안 글자와 그 아래 전동기 태그를 읽습니다.
5. 오른쪽의 다른 Feeder와 전동기 태그를 Feeder #C의 값으로 섞지 않습니다.
6. 각 답에는 이미지에서 직접 읽은 문자열과 연결을 확인한 순서를 함께 적습니다.
7. 읽히지 않는 문자는 추측하지 않고 `확인하지 못함`으로 적습니다.

마지막에는 네 항목을 표로 정리해 주세요.
```

프롬프트 파일로 실행하려면 [`단계별-질문.txt`](../실습자료/프롬프트/단계별-질문.txt)를 사용하세요.
