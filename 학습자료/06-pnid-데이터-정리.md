...
> **선택 실습입니다.** JSON 파일을 직접 만들지 않아도 예시를 읽고 색인의 구조를 이해한 뒤 다음 단원으로 진행할 수 있습니다.

> 이 단원에서는 P&ID의 주요 구역과 태그를 중심으로 정리합니다. 도면이 무엇을 나타내는지 빠르게 파악하고 필요한 위치를 다시 찾을 수 있는 수준이면 충분합니다.

앞 단원에서는 위치 상자로 이미지의 특정 지점을 다시 찾는 방법을 배웠습니다. 이번에는 그 방식을 도면 전체로 넓힙니다. 빈 JSON 파일에 도면 기본정보, 한 줄 요약, 큰 구역, 주요 태그를 한 단계씩 추가해 보겠습니다.

완성된 결과는 P&ID 전체를 디지털화한 모델보다 탐색용 색인에 가깝습니다. 사람이나 AI 에이전트가 도면을 열기 전에 <mark class="text-highlight">어떤 도면인지, 어떤 큰 구역이 있는지, 어디를 먼저 확대할지 판단</mark>하는 데 사용합니다.
...
파일명에는 이 도면이 설명하는 계통과 revision 정보가 빠져 있습니다. 내용을 파악하려면 매번 PDF를 열어야 합니다.
...
`title`은 도면의 공식 이름이고, `summary`는 이 시트에서 실제로 볼 수 있는 큰 내용을 짧게 설명합니다. 요약에는 주요 장비와 구조만 담습니다.
...
region은 장비 하나의 외곽을 정밀하게 따는 픽셀 마스크보다 넓은 개념입니다. 다시 살펴볼 의미가 있는 사각형 영역을 가리킵니다. 좌표는 원본 이미지 왼쪽 위를 원점으로 하는 `[x_min, y_min, x_max, y_max]` 형식으로 기록합니다.
...
구역 설명은 완전한 설비 목록보다 탐색 안내에 가깝습니다. 어떤 질문이 들어왔을 때 이 구역을 살펴봐야 하는지 판단할 수 있을 정도면 충분합니다.
...
여기서는 <mark class="text-highlight">어떤 태그가 이 도면에 있고, 어느 큰 구역에서 다시 확인할 수 있는지 기록</mark>합니다. 태그 사이의 세부 연결 관계는 필요한 경우 다음 단계에서 확장합니다.
...
직접 실습하려면 압축을 푼 `pnid-ai-workbook-kit` 폴더를 Codex의 작업 폴더로 열고 다음 요청을 입력합니다.

```text
이 단원의 완성된 도면 레코드를 `output/pnid-index/document.json`으로 저장해 주세요.

다음 항목이 빠짐없이 있어야 합니다.
- 도면번호, 제목, revision과 sheet
- 도면 한 줄 요약
- R01부터 R06까지 여섯 구역의 id, name, summary와 bbox
- R03에서 확인한 네 태그와 region 연결

저장 후 JSON 문법을 검사하고, 모든 bbox가 원본 이미지 4963×3509 픽셀 경계 안에 있으며
x_min < x_max, y_min < y_max인지 확인해 주세요. 원본 파일은 수정하지 마세요.
완료 응답에는 저장 경로와 검사 결과만 간단히 적어 주세요.
```

완료 후 `output/pnid-index/document.json`을 열어 도면번호 `J-11520-ZM-105-005`, revision `F`, 여섯 구역과 네 태그가 있는지 확인합니다. 이 네 가지가 보이고 JSON 검사를 통과했다면 실습을 완료한 것입니다.

## 여러 도면으로 확장할 수도 있습니다

지금은 P&ID 한 장만 정리했습니다. 여러 도면의 `title`, `summary`, `regions`, `tags`를 같은 형식으로 저장해 두면 AI 에이전트가 작업 전에 각 도면의 의미를 빠르게 파악할 수 있습니다.

예를 들어 질문에 `Reception Bin 2`나 `V186`이 들어 있다면 모든 PDF를 처음부터 자세히 읽는 대신 관련 도면과 region을 먼저 후보로 고를 수 있습니다. 이 워크북에서는 간단한 도면 레코드까지만 만들며, 이후 여러 문서를 검색하는 시스템과 에이전트 탐색 입력으로 확장할 수 있습니다.

## 다음 단원으로 넘어갑니다

이번 단원에서는 P&ID 한 장을 검색 가능한 레코드로 정리했습니다. 다음 단원에서는 전체 도면 탐색, 부분 확대, 근거 확인과 결과 기록을 하나의 업무 요청문으로 묶습니다.

## 단원 마무리

- **이번에 한 일:** 도면 기본정보, 요약, 구역과 태그를 같은 JSON 구조로 정리했습니다.
- **확인할 결과:** `document.json`에 도면번호, revision, 여섯 구역과 네 태그가 있고 JSON 검사를 통과합니다.
- **실패해도 괜찮은 항목:** 파일을 직접 만들지 않았다면 완성 예시의 각 필드 역할을 설명하는 것으로 충분합니다.
- **다음 단계:** 지금까지의 탐색과 검증 절차를 재사용 가능한 업무 요청문으로 묶습니다.
