...
unexpected_pdfs=$(grep -v '^실습자료/ocr/검색가능한-pnid.pdf$' <<<"$public_pdfs" || true)
...
author=$(pdfinfo 실습자료/ocr/검색가능한-pnid.pdf | sed -n 's/^Author:[[:space:]]*//p')
title=$(pdfinfo 실습자료/ocr/검색가능한-pnid.pdf | sed -n 's/^Title:[[:space:]]*//p')
